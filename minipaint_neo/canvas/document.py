...
Strokes and crop drags are undone by the editor itself. What this holds is the
structural work - receive, open, apply crop, expand, clear, reset - because
those replace the editor's contents wholesale, and the component's own history
does not survive that.

Every structural step goes through the same three moves: a callback *stages*
the next document in ``pending``; the browser is told to wind the editor back
to a clean state; a second callback *commits* ``pending`` and pushes it into
the editor. Staging first means the editor's current pixels are read before
anything is undone.
"""
...
class Document:
    ...
    def __init__(self) -> None:
        ...
        self.pending: typing.Optional[dict] = None
        self.last_send: str = ""
    ...
    def _snapshot(self, label: str) -> dict:
        ...
        slow anyway, and six uncompressed 4K RGBA frames is a lot of somebody's
        server memory per open tab.
        """
        return {
            "label": label,
    ...
    def stage(
        ...
        *,
        ...
        filename: typing.Optional[str] = None,
        expansion: typing.Optional[dict] = None,
        restore: typing.Optional[str] = None,
        mode: typing.Optional[str] = None,
        checkpoint: bool = True,
    ) -> None:
        """Queue the next document. Nothing changes until ``commit_pending``."""
        self.pending = {
            "label": label,
            "message": message,
    ...
    def commit_pending(self) -> typing.Optional[dict]:
        """Apply what was staged. Returns it, or None when nothing was."""
        pending, self.pending = self.pending, None
        if pending is None:
            return None

        if pending["restore"] == "undo":
            pending["label"] = self.undo() or pending["label"]
            return pending
        if pending["restore"] == "redo":
            pending["label"] = self.redo() or pending["label"]
            return pending

        if pending["checkpoint"]:
            self.checkpoint(pending["label"])

        if pending["load"]:
            self.load(pending["image"], pending["origin"] or "none", pending["filename"])
        else:
            self.commit(pending["image"], pending["mask"])
            if pending["expansion"] is not None:
                self.has_expansion = True
                self.expansion = dict(pending["expansion"])
        return pending
