"""Legacy miniPaint UI: the editor in an iframe, plus what its bridge needs.

Behaviour here is deliberately unchanged. The parent-frame half of the bridge
is ``javascript/main.js``; it only ever wakes up when this frontend mounts,
because it waits for the iframe to call ``a1111minipaint.onload()``.
"""
...
from .send_log import announce_send_log
