...
def run() -> Results:
    ...
    # after the mode: the menu button's label, 4 rail panels, the tool,
    # then the layer list, the size, the opacity, the name, the drag preview and underlay
    MENU_LABEL = MODE + 1
    TOOL = MODE + 6
    LAYER_LIST, LAYER_SCALE, LAYER_OPACITY, LAYER_NAME, LAYER_PREVIEW, LAYER_UNDERLAY = range(MODE + 7, MODE + 13)
    COMMIT_LEN = 2 + canvas.INFO_COUNT
    r.check("the commit shape is the two canvas values plus the information", COMMIT_LEN == 22 and canvas.MODE_COUNT == 13)
    ...
    r.check("receive selects crop mode, the menu button says so", out[MODE] == "crop" and value_of(out[MENU_LABEL]) == "☰ Crop")
    ...
    updates = canvas.request_mode("mask:1700000000", doc)
    r.check("mode updates have the mode shape", len(updates) == canvas.MODE_COUNT)
    r.check("the request picks the mode and the menu button follows", updates[0] == "mask" and value_of(updates[1]) == "☰ Mask")
    r.check("mask mode resets the tool", value_of(updates[6]) == "Paint")
    r.check("mask mode shows its rail panel only", [u.get("visible") for u in updates[2:6]] == [False, True, False, False])
    r.check("crop mode leaves the tool alone", skipped(canvas.set_mode("crop", doc)[6]))
    r.check("outside Layers mode the drag previews are not sent", skipped(updates[11]) and skipped(updates[12]))
    r.check("an unknown mode is crop", canvas.set_mode("nonsense", doc)[0] == "crop" and canvas.request_mode(None, doc)[0] == "crop")
    r.check("the layer list is only re-sent when it changed", skipped(canvas.set_mode("mask", doc)[7]))
    ...
    r.check("layers mode sends the drag previews: the picture, and the Background under it", isinstance(updates[11], str) and updates[11].startswith('{"src": "data:image/png;base64,') and '"name": "Layer 1"' in updates[11] and updates[12].startswith("data:image/png"))
    ...
    r.check("the preview covers both, over the layer between them", '"name": "Background, Layer 2"' in out[LAYER_PREVIEW] and out[LAYER_UNDERLAY].startswith("data:image/png"))
    ...
    doc = out[STATE]
    r.check("re-center brings the layer to the middle of the canvas", (doc.active_layer.x, doc.active_layer.y) == (50, 30) and "Centered Layer 2" in out[STATUS] and isinstance(out[BG], Image.Image))
    out = canvas.undo(doc, "layers")
    doc = out[STATE]
    ...
    out = canvas.layer_scale_by(2, bg, fg, doc, "layers")
    doc = out[STATE]
    r.check("double makes the layer twice as big about its centre, and reloads", doc.active_layer.size == (200, 120) and (doc.active_layer.x, doc.active_layer.y) == (-15, -25)
            and "Layer 2 at 200% size" in out[STATUS] and isinstance(out[BG], Image.Image) and out[LAYER_SCALE].get("value") == 200 and "200% size" in out[LAYER_LIST])
    out = canvas.layer_scale(50, bg, fg, doc, "layers")
    doc = out[STATE]
    ...
    r.check("the same size again changes nothing and leaves no undo step", skipped(canvas.layer_scale(50, bg, fg, doc, "layers")[BG]) and doc.history[-1]["label"] == "resize layer")
    out = canvas.layer_scale_by(None, bg, fg, doc, "layers")
    doc = out[STATE]
    ...
    r.check("a size no browser canvas holds is refused", "keep a side under" in canvas.layer_scale(400, bg, fg, canvas.receive([(Image.new("RGB", (3000, 3000)), None)], None, "layers", "txt2img")[STATE], "layers")[STATUS])
    r.check("nonsense is ignored", skipped(canvas.layer_scale("big", bg, fg, doc, "layers")[BG]))
    out = canvas.undo(doc, "layers")
    doc = out[STATE]
    r.check("undo takes a resize back", "Undid resize layer" in out[STATUS] and doc.active_layer.size == (50, 30))
    doc = canvas.layer_scale_by(None, bg, fg, doc, "layers")[STATE]

    out = act("down", "Layer 2", doc)
    doc = out[STATE]
    r.check("the arrow reorders", doc.layer_names() == ["Background", "Layer 2", "Layer 1"] and doc.active == 1 and "Layer 2 moved down" in out[STATUS] and isinstance(out[BG], Image.Image))
    ...
    doc = out[STATE]
    r.check("the eye hides a layer and reloads the composite", not doc.layers[0].visible and "Background hidden" in out[STATUS] and isinstance(out[BG], Image.Image))
    ...
    r.check("the bottom layer has nothing to merge into", "nothing below Background" in canvas.layer_merge(bg, fg, doc, "layers")[STATUS])
    out = canvas.undo(doc, "layers")
    doc = out[STATE]
    ...
    r.check("a masked area becomes a layer, trimmed to it, and the mode switches to Layers",
            doc.active_layer.name == "Layer 3" and doc.active_layer.size[0] < 200 and out[MODE] == "layers" and value_of(out[MENU_LABEL]) == "☰ Layers" and skipped(out[BG]))
    ...
    out = canvas.send(bg, fg, doc, "mask", "inpaint:1700000001", "Off")
    doc = out[SENT_DOC]
    instruction, payload = out[-2], out[-1]
    r.check("inpaint names the size its canvas must reach", instruction == "inpaint:200x120")
    r.check("the request's nonce is dropped, and no request at all takes the suggestion", canvas.send(bg, fg, doc, "mask", "", "Off")[-2] == "inpaint:200x120")
    ...
    out = canvas.send(bg, fg, doc, "mask", "inpaint", "Medium")
    ...
    out = canvas.send(bg, fg, doc, "mask", "img2img:1700000002", "Off")
    ...
    out = canvas.send(bg, fg, doc, "mask", "stitch_txt2img:1700000003", "Off")
    ...
    out = canvas.send(bg, fg, doc, "mask", "stitch_img2img", "Off")
    ...
    out = canvas.send(bg2, fg, holed, "layers", "img2img", "Off")
    ...
    out = canvas.send(bg2, fg, holed, "layers", "stitch_txt2img", "Off")
    r.check("ImageStitch too", out[STITCH_T2I][0].getpixel((150, 100))[3] == 0)
    out = canvas.send(bg2, fg, holed, "layers", "extras", "Off")
    ...
    shared.opts.data[settings.SEND_FILL] = "Neutral gray"
    out = canvas.send(bg2, fg, holed, "layers", "img2img", "Off")
    r.check("the setting can fill them with a colour instead", decode_data_url(out[-1]).getpixel((150, 100)) == (127, 127, 127, 255) and "filled with rgb(127, 127, 127)" in out[SENT_STATUS])
    ...
    bg = imaging.to_rgba(doc.image)
    out = canvas.send(bg, layer(doc.mask, doc.size), doc, "mask", "", "Off")
    ...
    out = canvas.send(bg, layer(doc.mask, doc.size), doc, "mask", "", "Off")
    sent = decode_data_url(out[-1])
    r.check("or filled, by the setting", sent.getchannel("A").getextrema() == (255, 255) and "filled with rgb(127, 127, 127)" in out[SENT_STATUS])
    shared.opts.data[settings.SEND_FILL] = settings.KEEP_TRANSPARENT
