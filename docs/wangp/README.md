...
| the WanGP tab is not the one on screen | this is the ordinary case, not a fault: the Send menu is on the Mini Paint tab, so the WanGP iframe is a document the browser is not rendering, and a document that is not rendered is given no animation frames. Gradio schedules every event trigger inside one, so a click on the bridge's hidden trigger would wait until the WanGP tab is opened again. The bridge script gives every frame requested while one of its requests is in flight a timer as well, and whichever fires first runs it — a rendered page's own frame always wins, a hidden page's timer answers. WanGP's own focus patch does the same for its hidden main tab, but needs a measurable panel, which a `display:none` iframe never has. |
