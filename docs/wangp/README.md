...
settings as they change. Bridge 1.8.0 adds the session guard: every acknowledgement carries
`session: {closed, reset, silent_s}` - Gradio's own closed flag, the bridge's finding that
...
| the WanGP tab is not the one on screen | since 2026-09-25 this is not a document the browser is not rendering: the tab's panel is *parked* rather than hidden (`style.css`, the rule keyed on `#tab_wangp[style*="display: none"]`), so the WanGP page keeps its layout box and its animation frames while another tab is selected, and the journal says `tab: the WanGP tab left the screen; its page is parked - laid out, invisible` and, on return, `tab: the WanGP tab is on screen again after Ns parked; meanwhile its page ran N frames by the browser and M by the bridge's timer`. Those counts are the measurement: Chromium keeps N high and M near zero for a parked page, Firefox does the opposite (it throttles a document whose frame is invisible), so on Firefox the parking buys geometry - no relayout on return - and not frames. `docs/wangp/PARKED_PANEL_2026-09-25.txt` says why, and what the freeze it was built for turned out to need instead. A page whose stylesheet never arrived is back in the old state, where the WanGP iframe is a document the browser is not rendering, and some browsers give such a document no animation frames at all. Gradio schedules every event trigger inside one and gates it on its component flush, which Gradio's core schedules through a reference captured when its module loaded - so a click on the bridge's hidden trigger would wait until the WanGP tab is opened again. The bridge therefore still places a frame timer in the page head, before Gradio's modules, so that every frame Gradio asks for is also given a timer: a rendered page's own frame always wins, a hidden page's timer answers. The plugin's log says `frame timer script placed in the page head` at startup; if it says the script could not be placed, the page script installs the timer late, the iframe's console says `frame timer installed late - Gradio's own flush is not covered`, and a query from the Mini Paint tab may then wait until the WanGP tab is opened. WanGP's own focus patch reschedules the same frames for its hidden main tab, and needs a measurable panel - which a parked panel is, and a `display:none` one never was. |
| `WanGP's page lost its session` on the tab, with *Reload view* and *Dismiss* | the bridge (1.8.0) found the page's Gradio session state reset: its heartbeat stream had dropped, Gradio marked the session closed and deleted its state an hour on, and WanGP's handlers have been running on a copy of the build-time state since - the page answers everything and does nothing, which is what "the frame froze" was. The view reloads itself within the same bounds as *WanGP stopped responding*; the log says `session: WanGP's page lost its session - Gradio deleted its state after its heartbeat dropped (last heard Ns ago)`, and the plugin's own console says `session: a page's session state was reset by Gradio`. With the guard installed this should be rare: the console says `session guard: Gradio's heartbeat route is wrapped` at the first request, `session: a page's heartbeat reconnected after Ns and its session is reopened` for every blip it caught, and the page journal says `session: the WanGP page's heartbeat stream dropped once and Gradio marked its session closed ... the bridge keeps its state` when one was caught while the page was on screen. If the guard could not be installed the console says why (`session guard: not installed (...)`), and a blip then does what Gradio does by itself. |
| the menu line was clicked and nothing arrived in WanGP | a send leaves a line at every step, in order, in `logs/wangp-log.txt` (column `send` for this side, `browser` for the page, `wangp` for the plugin): `send: start_frame chosen from the menu` (the click reached the Canvas script), `WanGP Start Frame: prepared WxH; the browser is handed the file` (the Canvas's send event ran; `logs/send-log.txt` has the same `prepared` entry), `deliver: the server prepared the picture for start_frame` (the browser step ran; when Gradio's chained step does not hand the instruction over, the Canvas reads it from its own boxes within a second and the line reads `deliver: the prepared file was read from the page's own boxes`, and when the boxes stay empty for a second and a half the Canvas presses its hidden fetch button, which asks the server for the prepared send again through an event that writes only those two boxes; the journal then shows `asking the server for the prepared file again` and, from this side, `the page asked for the prepared file again; handed over`; a `deliver: gave up after 90 s` line means neither route ever put the answer in the boxes), `WANGP_RECEIVE_IMAGE: asked (id) for start_frame` (the page was asked), `[wan2gp-minipaint-bridge] receive: start_frame replace pixel-equivalent in N ms` (the plugin placed it), `WANGP_RECEIVE_RESULT: answered after N ms` and `the browser reports: start_frame taken`. The WanGP tab then opens by itself. The first line missing names the half that stopped: no `chosen from the menu` line means the click never reached the script (the browser console is the place to look); no `prepared` line means the Canvas's send event never ran or raised, and the Forge console then has the traceback; `send: refused before asking - CODE` is the browser declining to ask, with the reason; `asked` without a plugin `receive:` line means the message never became a Gradio event inside the iframe, and the iframe's own console says why (`receive: refused before the click - CODE`, `dropped WANGP_RECEIVE_IMAGE: channel …`, or `receive: queued` with nothing after it); a `receive:` line without a result is an acknowledgement that never came back, and the browser writes `no acknowledgement within 30000 ms` when it gives up. |
