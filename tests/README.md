...
| `test_frontends.py` | the Old UI setting exists, is saved, defaults to the new editor and asks for a Reload UI; each frontend mounts alone, under the same tab name and id; a new UI that throws still leaves a working legacy tab and says why; arguments this Gradio does not have are filtered rather than raised |
