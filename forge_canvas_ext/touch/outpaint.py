...
def describe(
    ...
    if size is None:
        return "No image yet."
    if not any(sides):
        return f"{size[0]} x {size[1]} - pick a side to expand."

    new_width, new_height = resulting_size(size, sides)
    line = f"{size[0]} x {size[1]}  ->  {new_width} x {new_height}"
...
def expand(
    ...
    that into the status line rather than this deciding how to word it.
    """
    ...
    if max(new_width, new_height) > HARD_LIMIT_SIDE:
        raise ValueError(
            f"{new_width} x {new_height} has a side over {HARD_LIMIT_SIDE}px, which is "
            "past what a browser canvas can hold. Expand less, or crop first."
        )
    if new_width * new_height > HARD_LIMIT_PIXELS:
        raise ValueError(
            f"{new_width} x {new_height} is {imaging.megapixels((new_width, new_height))} "
    ...
    if fill == "Transparent":
        background = (0, 0, 0, 0)
    elif fill == "Edge (stretch)":
        background = (0, 0, 0, 0)
    else:
        background = imaging.fill_colour(fill, source) + (255,)
