...
Protocol 3 adds the queue operation: a request that overlays a prompt and up
to three kinds of image on the live WanGP page and asks that page to add
itself to WanGP's own queue. The rule that defines it is written once, in
``normalize_queue_request``: a field the caller leaves out is inherited from
the page, a field the caller supplies overrides the page for that one queued
request, and nothing else - no path, no model, no setting - may travel.
