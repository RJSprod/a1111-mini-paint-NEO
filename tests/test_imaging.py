...
def run() -> Results:
    ...
    r.check("the drag preview describes the active layer", '"w": 20' in doc.preview_payload() and doc.underlay_payload().startswith("data:image/png"))
    r.check("flatten leaves one layer", doc.flatten() == 2 and doc.layer_names() == ["Background"] and doc.underlay_payload() == "")
